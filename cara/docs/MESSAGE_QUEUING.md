...
The state is managed in `CaraWeb.ChatLive` using the following assigns:
- `active_task`: The PID of the asynchronous task handling the LLM request.
- `pending_messages`: A list of strings representing messages waiting for processing.
- `current_user_message`: The message currently being answered by the AI.
