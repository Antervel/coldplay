defmodule Cara.AI.ChatTest do
  ...
  describe "guard integration" do
    ...
    test "blocks unsafe student message before reaching LLM", %{bypass: bypass} do
      # Mock guard endpoint (unsafe)
      ...
      context = Chat.new_context("Test system prompt")
      {:ok, response, new_context} = Chat.send_message("Unsafe content", context, model: "openai:test-model")

      assert response == "Violent Crimes"
      assert length(new_context.messages) == 3
      assistant_messages = Enum.filter(new_context.messages, &(&1.role == :assistant))
      assert hd(hd(assistant_messages).content).text == "Violent Crimes"
    end

    test "blocks unsafe LLM response", %{bypass: bypass} do
      Bypass.expect(bypass, "POST", "/chat/completions", fn conn ->
        {:ok, body, conn} = Plug.Conn.read_body(conn)
        request = Jason.decode!(body)
        text = request["messages"] |> hd() |> Map.get("content")

        if text == "Tell me something" do
          # First call (student message) - safe
          Plug.Conn.send_resp(
            conn,
            200,
            Jason.encode!(%{
          ...
          # Second call (LLM response) - unsafe
          Plug.Conn.send_resp(
            conn,
            200,
            Jason.encode!(%{
      ...
      Bypass.expect(bypass, "POST", "/v1/chat/completions", fn conn ->
        ...
        response = %{
          ...
          "choices" => [
            %{
              "index" => 0,
              "delta" => %{"content" => "Potentially unsafe content"},
              "finish_reason" => nil
            }
          ]
        }
      ...
      context = Chat.new_context("Test system prompt")
      {:ok, response, new_context} = Chat.send_message("Tell me something", context, model: "openai:test-model")

      assert response == "Sorry, I can't answer about this topic. What else do you want to know about?"
      assert length(new_context.messages) == 3
    end

    test "allows safe messages and returns LLM response", %{bypass: bypass} do
      # Mock guard - safe
      Bypass.expect(bypass, "POST", "/chat/completions", fn conn ->
        response =
          Jason.encode!(%{
